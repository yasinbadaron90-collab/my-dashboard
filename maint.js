...
function renderMaintCard(){
  // Defensive guards — callers may invoke this before PASSENGERS / fmtR /
  // getMaintData are ready. Failing silently leaves "Loading..." stuck on
  // the card; better to bail and let a later caller succeed.
  if(typeof PASSENGERS === 'undefined' || !Array.isArray(PASSENGERS)) {
    console.debug('[renderMaintCard] PASSENGERS not ready, deferring');
    return;
  }
  if(typeof fmtR !== 'function' || typeof getMaintData !== 'function') {
    console.debug('[renderMaintCard] dependencies not ready, deferring');
    return;
  }
