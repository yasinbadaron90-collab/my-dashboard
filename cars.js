...
function renderCars(){
  // Filter soft-deleted cars — they're hidden during the undo window
  var cars = loadCarsData().filter(function(c){ return !c._deleted; });
  ...
  if(cars.length === 0){
    ...
    cars.forEach(function(car){
      ...
      // Expense rows — show 5 most recent by default, hide the rest behind
      // a toggle. Avoids scrolling through 50+ rows just to see other cars.
      var rowsHtml = '';
      if(expenses.length === 0){
        ...
        var listId = 'expList_'+car.id;
        var btnId  = 'expBtn_' +car.id;
        var hiddenWrapId = 'expHidden_'+car.id;

        // Renders one expense row — shared between visible and hidden lists.
        ...
        // Remaining — wrapped in a div that's hidden until user clicks toggle
        if(hasMore){
          rowsHtml += '<div id="'+hiddenWrapId+'" style="display:none;">';
          sortedExp.slice(SHOW_DEFAULT).forEach(function(e){ rowsHtml += buildExpRow(e); });
          rowsHtml += '</div>';
          ...
          rowsHtml += '<div style="padding:10px 16px;text-align:center;border-top:1px solid #1a1a1a;">'
            + '<button id="'+btnId+'" onclick="toggleExpList(\''+hiddenWrapId+'\',\''+btnId+'\','+moreCount+')" '
...
// ── Expense list show more / show less toggle ──
// Called by the "Show all (X more)" button rendered alongside each car's
// expense list. Flips the hidden div's display and rewrites the button
// text to match the new state.
function toggleExpList(hiddenWrapId, btnId, moreCount){
...
function deleteCar(carId){
  ...
  var allCars = loadCarsData();
  var car = allCars.find(function(c){ return c.id === carId; });
  if(!car) return;
  // Soft-delete: flag the car. renderCars filters _deleted entries so
  // it vanishes from the list during the 8-second undo window.
  car._deleted = true;
  saveCarsData(allCars);
  renderCars();
  ...
  softDeleteToast({
    ...
    onUndo: function(){
      ...
      var c = cars2.find(function(x){ return x.id === carId; });
      if(c){ delete c._deleted; saveCarsData(cars2); }
      renderCars();
    },
    onPurge: function(){
      var cars2 = loadCarsData();
      var newCars = cars2.filter(function(c){ return c.id !== carId; });
      saveCarsData(newCars);
    }
  });
}
