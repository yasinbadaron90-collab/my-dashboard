...
(function(){
  ...
  adapter.uploadLocal = async function(){
    ...
    // Delete all existing entries for this household first, then re-insert.
    // This prevents stale duplicate rows from old passenger_id mappings.
    var delRes = await window.sb.from('carpool_entries').delete().eq('household_id', hh);
    ...
    for(var i=0;i<rows.length;i+=chunkSize){
      ...
      var res = await window.sb.from('carpool_entries').insert(chunk);
      if(res.error) throw res.error;
      total += chunk.length;
    }
...
(function(){
  ...
  adapter.uploadLocal = async function(){
    ...
    // Deduplicate events by date+type+title+time before uploading
    var seenEvents = {};
    ...
    // Save deduped array back to localStorage
    _write(EVENTS_KEY, events);
    ...
    if(eventRows.length){
      // Delete all existing school events first, then re-insert clean
