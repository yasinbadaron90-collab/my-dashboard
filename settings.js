...
// Formula: baseline + sum(tagged income to this bank) − sum(tagged expense
//          from this bank) − sum(tagged savings allocations from this bank).
// Untagged legacy entries (no destBank, no recognisable account) are ignored
// — they show as "—" in the UI and don't move bank totals.
function _computeBankBucket(bank, cfData, baseline){
  var net = baseline || 0;
  ...
  Object.keys(cfData).forEach(function(mk){
    ...
    (month.income || []).forEach(function(e){
      if(_entryBank(e) === bank) net += Number(e.amount) || 0;
    });
  ...
  // Apply recurring entries to the CURRENT month (calendar) too. Recurring
  // are "always on" so they should affect today's balance.
  var rec = cfData.recurring || {};
  (rec.income || []).forEach(function(e){
    if(_entryBank(e) === bank) net += Number(e.amount) || 0;
  });
  (rec.expenses || []).forEach(function(e){
    if(_entryBank(e) === bank) net -= Number(e.amount) || 0;
  });
  return net;
}
...
function renderBankBalanceCard(){
  ...
  if(!amtEl) return; // card not on the page

  // Baseline: prefer live values from the input fields if present, fall back
  // to saved values. Lets the running total respond to in-flight edits.
  ...
  var fnbBase  = (fnbInp  && fnbInp.value  !== '') ? (parseFloat(fnbInp.value)  || 0) : (saved.fnb  || 0);
  var tymeBase = (tymeInp && tymeInp.value !== '') ? (parseFloat(tymeInp.value) || 0) : (saved.tyme || 0);
  var cashBase = (cashInp && cashInp.value !== '') ? (parseFloat(cashInp.value) || 0) : (saved.cash || 0);

  // Live balances: baseline + tagged income − tagged expenses/savings
  var cfData = (typeof loadCFData === 'function') ? loadCFData() : {};
  var fnb  = _computeBankBucket('FNB',      cfData, fnbBase);
  var tyme = _computeBankBucket('TymeBank', cfData, tymeBase);
  var cash = _computeBankBucket('Cash',     cfData, cashBase);
  var total = fnb + tyme + cash;
  ...
  // Legacy breakdown text (kept for back-compat with any other code reading it)
  if(brkEl) brkEl.textContent = 'FNB: '+fmtR(fnb)+' · Tyme: '+fmtR(tyme)+' · Cash: '+fmtR(cash);

  // Subtle color cue if everything is zero — let the user know the card is
  // waiting for data, not broken.
  if(total <= 0 && fnbBase <= 0 && tymeBase <= 0 && cashBase <= 0){
...
function renderCashFlow(){
  ...
  if(summaryCard){
    ...
    if(_sa){ _sa.style.color = netColor; _sa.textContent = (net>=0?'+':'')+fmtR(net); }
